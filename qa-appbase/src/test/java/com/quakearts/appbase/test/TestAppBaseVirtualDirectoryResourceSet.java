...
public class TestAppBaseVirtualDirectoryResourceSet {
	...
	public void testMethods() throws Exception {
		...
		resourceSet.addUrl("tomcat-embed-el-9.0.8.jar", impl.getLibraryPath("tomcat-embed-el-9.0.8.jar"));
		resourceSet.addUrl("tomcat-embed-websocket-9.0.8.jar", impl.getLibraryPath("tomcat-embed-websocket-9.0.8.jar"));
		resourceSet.start();
		
		assertThat(resourceSet.getLibraryUrls().size(), is(2));
		assertThat(resourceSet.getLibraryUrls().contains(impl.getLibraryPath("tomcat-embed-el-9.0.8.jar")), is(true));
		assertThat(resourceSet.getLibraryUrls().contains(impl.getLibraryPath("tomcat-embed-websocket-9.0.8.jar")), is(true));
		
		WebResource resource = resourceSet.getResource("/WEB-INF/lib/tomcat-embed-websocket-9.0.8.jar");
		...
		assertThat(resource.getURL(), is(impl.getLibraryPath("tomcat-embed-websocket-9.0.8.jar")));
		
		List<String> list = Arrays.asList(resourceSet.list("/WEB-INF/lib/"));
		assertThat(list.size(), is(2));
		assertThat(list.contains("tomcat-embed-el-9.0.8.jar"), is(true));
		assertThat(list.contains("tomcat-embed-websocket-9.0.8.jar"), is(true));
		
		assertThat(resourceSet.list("/WEB-INF/lib/resource").length, is(0));
		
		Set<String> set = resourceSet.listWebAppPaths("/WEB-INF/lib/");
		assertThat(set.size(), is(2));
		assertThat(set.contains("/WEB-INF/lib/tomcat-embed-el-9.0.8.jar"), is(true));
		assertThat(set.contains("/WEB-INF/lib/tomcat-embed-websocket-9.0.8.jar"), is(true));		
	}
}
