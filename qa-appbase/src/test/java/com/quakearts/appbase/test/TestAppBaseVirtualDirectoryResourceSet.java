...
public class TestAppBaseVirtualDirectoryResourceSet {
	...
	public void testMethods() throws Exception {
		...
		assertThat(resourceSet.getLibraryUrls().contains(impl.getLibraryPath("tomcat-embed-el-9.0.48.jar")), is(true));
		assertThat(resourceSet.getLibraryUrls().contains(impl.getLibraryPath("tomcat-embed-websocket-9.0.48.jar")), is(true));
		
		WebResource resource = resourceSet.getResource("/WEB-INF/lib/tomcat-embed-websocket-9.0.48.jar");
		...
		assertThat(resource.getURL(), is(impl.getLibraryPath("tomcat-embed-websocket-9.0.48.jar")));
		
		List<String> list = Arrays.asList(resourceSet.list("/WEB-INF/lib/"));
		assertThat(list.size(), is(2));
		assertThat(list.contains("tomcat-embed-el-9.0.48.jar"), is(true));
		assertThat(list.contains("tomcat-embed-websocket-9.0.48.jar"), is(true));
		
		assertThat(resourceSet.list("/WEB-INF/lib/resource").length, is(0));
		
		Set<String> set = resourceSet.listWebAppPaths("/WEB-INF/lib/");
		assertThat(set.size(), is(2));
		assertThat(set.contains("/WEB-INF/lib/tomcat-embed-el-9.0.48.jar"), is(true));
		assertThat(set.contains("/WEB-INF/lib/tomcat-embed-websocket-9.0.48.jar"), is(true));		
	}
}
