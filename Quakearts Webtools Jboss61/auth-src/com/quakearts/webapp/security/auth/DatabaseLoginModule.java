...
public class DatabaseLoginModule implements LoginModule {
	...
	private static final Logger log = Logger
			.getLogger(DatabaseLoginModule.class);
	...
	public void initialize(Subject subject, CallbackHandler callbackHandler,
		...
		if (iterationsValue != null)
			try {
				...
				log.error("Invalid value for hash_interations. Using default",
						e);
				iterations = 10;
			}
		...
		try {
			...
			if (!load_profile_only) {
				...
				if (log.isTraceEnabled())
					log.trace("Verifying subject " + username);
				...
				rs.close();
				stmt.close();
				log.trace("Subject " + username + " verified.");
