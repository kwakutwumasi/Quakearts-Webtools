...
public abstract class CurrentSessionContextHelper implements CurrentSessionContext {
	...
	private Session getCurrentSessionFromContextAttributes() {
		return (Session) getContextAttributes().get(CURRENTSESSION+(domain!=null?":"+domain:EMPTY));
	}

	private void removeCurrentSessionFromContextAttributes() {
		getContextAttributes().remove(CURRENTSESSION+(domain!=null?":"+domain:EMPTY));
	}
	...
	private Session createCurrentSession(){							
		try {
			...
			getContextAttributes().put(CURRENTSESSION+(domain!=null?":"+domain:EMPTY), session);
			return session;
